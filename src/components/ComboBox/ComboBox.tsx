...
import {
  handleFilter as handleFilterFunc,
  searchItem as searchItemFunc,
...
const ComboBox: React.FC<Props> = (props: Props) => {
  ...
  const searchItem: (key: string, groups: IComboBoxGroup[]) => IComboBoxItem | undefined =
    useCallback((key, groups) => searchItemFunc(key, groups), []);
  ...
  const handleItemFocus = useCallback(() => {
    const listItems: NodeListOf<any> = containerRef?.current?.querySelectorAll(
      'li[role="menuitemradio"][aria-disabled="false"]'
    );
    if (listItems?.length) {
  ...
  const handleInputKeyDown = useCallback(
    (event) => {
      ...
      if (
        (!isOpen && event.code === EVENT.KEY.KEYCODE.ENTER) ||
        ...
        {isOpen ? (
          <div className={STYLE.selectionPosition} ref={selectionPositionRef}>
            <div
              ...
              ref={menuRef}
              style={{ maxHeight: `${selectionContainerMaxHeight}px` }}
            >
              <Menu
              ...
              >
                {groups.length ? (
                  ...
                  <Item key={KEYS.INPUT_SEARCH_NO_RESULT} textValue={noResultText}>
                    <div aria-label={STYLE.noResultText} className={STYLE.noResultText}>
                      {noResultText}
                    </div>
                  </Item>
                )}
