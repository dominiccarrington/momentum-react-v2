...
`<ComboBox />` The component is composed of three parts: Button, Menu and Input. In the component,
the comboBoxGroups passed in are filtered based on the value of the Input and then passed to the
internal Menu to implement the search function.
...
If not controlled, it may confuse the user. Therefore, it is suggested to perform dynamic rendering
only when the list is closed.
...
An iterable list of sectionGroups is can be passed to the ComboBox using the comboBoxGroups prop. It
should be noted that the outermost array is used to distinguish the group of sections, and the items
within the section accept an iterable list of options.
...
ComboBox supports sections in order to group options. Sections can be used by wrapping groups of
